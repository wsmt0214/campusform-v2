...
public class CommentService {
    ...
    public CommentCreateResponse createComment(Long projectId, Long applicantId, Long authorId,
        ...
        if (request.getParentId() != null) {
            ...
            comment =
                    Comment.createReply(parent, applicantId, authorId, stage, request.getContent());

            // parent가 제대로 설정되었는지 확인
            if (comment.getParent() == null
                    || !comment.getParent().getId().equals(request.getParentId())) {
                throw new IllegalStateException(
                        "부모 댓글 설정에 실패했습니다. parentId: " + request.getParentId());
            }
        } else {
            // 루트 댓글 작성
    ...
    /**
     * 댓글 생성 이벤트 발행. 해당 프로젝트의 모든 관리자(OWNER + ADMIN)에게 알림이 가며, 댓글 작성자 본인에게는 발송하지 않습니다.
    ...
    private void publishCommentCreatedEvent(Long applicantId, Long commenterId,
        ...
        var commenter = userRepository.findById(commenterId).orElse(null);

        // 수신자: 해당 프로젝트의 모든 관리자(OWNER + ADMIN) 중 댓글 작성자 본인만 제외
        List<Long> recipientIds =
                project.getAdminIds().stream().filter(id -> !id.equals(commenterId)).toList();
        ...
        String commenterName = commenter != null && commenter.getNickname() != null
                && !commenter.getNickname().isBlank() ? commenter.getNickname()
                        : (commenter != null && commenter.getEmail() != null ? commenter.getEmail()
                                : "알 수 없음");
        ...
        eventPublisher.publishEvent(new CommentCreatedEvent(project.getId(), projectTitle,
                applicantId, applicant.getName() != null ? applicant.getName() : "지원자", commenterId,
                commenterName, recipientIds, stage.name()));
    }
    ...
    public CommentUpdateResponse updateComment(Long projectId, Long applicantId, Long commentId,
        ...
        if (!comment.getStage().equals(stage)) {
            throw new IllegalArgumentException(String.format(
                    "해당 모집 단계의 댓글이 아닙니다. 댓글의 stage: %s, 요청한 stage: %s", comment.getStage(), stage));
        }
    ...
    /**
     * 댓글 삭제 (프로젝트 관리자 권한 검증 후, 작성자 본인만 삭제 가능). 부모 댓글 삭제 시 해당 댓글의 모든 대댓글(직접·간접)은 엔티티의 cascade = ALL
     * + orphanRemoval = true 로 DB에서 함께 삭제된다.
     */
    public void deleteComment(Long projectId, Long commentId, Long authorId,
            RecruitmentStage stage) {
        projectAuthorizationService.assertAdmin(projectId, authorId);
    ...
    /**
     * 특정 단계의 지원자에 달린 댓글 조회 (프로젝트 OWNER/ADMIN만 조회 가능)
     */
    @Transactional(readOnly = true)
    public List<CommentResponse> getComments(Long projectId, Long applicantId,
        ...
        Map<Long, String> authorIdToNickname = authors.stream().collect(
                Collectors.toMap(User::getId, u -> u.getNickname() != null ? u.getNickname() : ""));
    ...
    private List<CommentResponse> buildCommentHierarchy(List<Comment> allComments,
        ...
        Map<Long, CommentResponse> commentMap = allComments.stream().collect(Collectors.toMap(
                Comment::getId,
                comment -> new CommentResponse(comment.getId(), comment.getAuthorId(),
        ...
        return rootComments.stream().map(comment -> commentMap.get(comment.getId()))
    ...
    /** 지원자가 해당 프로젝트에 속하는지 검증 (URL projectId와 지원자 소속 프로젝트 일치) */
    private void validateApplicantBelongsToProject(Long applicantId, Long projectId) {
        Long applicantProjectId = applicantRepository.findById(applicantId)
                .orElseThrow(() -> new EntityNotFoundException("존재하지 않는 지원자입니다.")).getProjectId();
