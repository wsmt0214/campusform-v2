...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    List<Comment> findAllByProjectIdAndStageOrderByCreatedAtAsc(@Param("projectId") Long projectId, @Param("stage") RecruitmentStage stage);

    /**
     * 프로젝트의 모든 지원자에 대한 댓글 조회 (대댓글 포함) - stage 필터링 없음 (하위 호환성)
     * 생성일시 오름차순 정렬
     */
    @Query("SELECT c FROM Comment c " +
           "JOIN Applicant a ON c.applicantId = a.id " +
