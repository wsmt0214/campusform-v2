...
- 泥?諛고룷?먯꽌??`nginx` 서비스媛 ?몄쬆?쒓? ?놁뼱???쒕?濡????????덈떎.
...
- `db` 서비스媛 `healthy` ?곹깭?몄? ?뺤씤
- `app` 서비스媛 醫낅즺?섏? ?딅뒗吏 ?뺤씤
- `nginx` 서비스媛 醫낅즺?섏? ?딅뒗吏 ?뺤씤
...
2. ?몄쬆??寃쎄퀬 ?놁쓬
3. `nginx` 서비스媛 ?ъ떆??諛섎났 ?놁씠 ?좎???
4. `app` 서비스媛 ?ъ떆??諛섎났 ?놁씠 ?좎???
5. `db` 서비스媛 `healthy` ?곹깭
6. Google 濡쒓렇???숈옉
